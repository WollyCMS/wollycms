...
export function seedMenus(
  ...
  // ---- Main Navigation ----
  const mainMenu = db.insert(menus).values({
    name: 'Main Navigation',
    slug: 'main',
  }).returning().get();

  const mainItems = buildMainMenu(mainMenu.id, pageMap);
  db.insert(menuItems).values(mainItems).returning().all();

  // ---- Top Bar ----
  const topMenu = db.insert(menus).values({
    name: 'Top Bar',
    slug: 'top',
  }).returning().get();
  ...
  // ---- Footer ----
  const footerMenu = db.insert(menus).values({
    name: 'Footer',
    slug: 'footer',
  }).returning().get();
 ...
 */
export function seedMainMenuChildren(
  db: AppDatabase,
  mainMenuId: number,
  pageMap: Record<string, number>,
) {
  ...
  const admChildren = db.insert(menuItems).values([
